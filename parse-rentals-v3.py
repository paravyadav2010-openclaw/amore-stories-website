...
def parse_snapshot(file_path, suburb_name):
    ...
    # Each property is in a listitem block
    listitem_blocks = re.findall(r'- listitem:(.*?)(?=- listitem:|$)', content, re.DOTALL)

    for block in listitem_blocks:
        # Extract address from heading - look for heading ending with suburb name
        heading_match = re.search(r'heading "([^"]+),\s*' + suburb_name + '"', block)
        if not heading_match:
            continue
        address = heading_match.group(1)

        # Extract price (look for paragraph with dollar amount)
        ...
        url_match = re.search(r'/url:\s+(https://www\.domain\.com\.au/[a-z0-9\-]+-' + suburb_name.lower() + r'-vic-[0-9]+)', block)
